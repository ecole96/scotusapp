...
def get_training_data(c,binary):
    ...
    c.execute("""SELECT * FROM rejectedTrainingData WHERE id <= 26301""")
    rows = c.fetchall()
    for r in rows:
...
def test_practical(c,n,binary):
    ...
    for run in range(n):
        ...
        for p in predict_probs:
            ...
            url = Xtest_raw[i][3]
            text = Xtest_raw[i][1]
            code, class_score = relevancy_check(clf,p,text,title,source,url,keywords)
            if binary and code in ['F','S']:
                code = "U"
            Y_process.append(code)
            i += 1
...
def relevancy_check(clf,probabilities,text,title,source,url,keywords):
    ...
    if any(term in title.lower() for term in (instantTerms + justice_keys)) or source in instantSources:
        ...
        code = "S"
        class_score = 1.0
    elif foreignCourtDetected(source,url,text,title):
        code = "F"
        class_score = 1.0
    else:
        code, class_score = classify(clf,probabilities,keywords,justice_keys)
    return code, class_score
...
def foreignCourtDetected(source,url,text,title):
...
def classify(clf,probabilities,keywords,justice_keys):
    prob_by_class = dict(zip(clf.classes_,probabilities)) # zip probabilities to their according class
    ...
    code = sorted_probs[0][0] # initial / highest classification
    class_score = sorted_probs[0][1] # highest probability
    if code == 'R': 
        ...
        if class_score < relevancy_threshold:
            code = sorted_probs[1][0] # if article fails threshold test, then finally classify it as the second most likely class
            class_score = sorted_probs[1][1] 
    return code, class_score
